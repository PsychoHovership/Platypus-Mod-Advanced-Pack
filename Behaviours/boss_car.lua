...
function OnInitialise()
    ...
    -- Predamage the boss
    if cartype ~= 4 then self.hitPoints = (self.hitPoints/2) + 150 end
...
function SetupVisuals()
    ...
    if cartype == 1 then
        ...
        bigGunAnimator.position = {x=-3, y=71 + 15}
    end
...
function OnTick()
    ...
    if cartype == 1 then
        ...
        if firewait == 0 then
            ...
            for llp = 60, 120, 15 do
                CreateBullet("enemyshot_car_cannon", self.worldPosition.x - 5, self.worldPosition.y + 125 + 15, math.cos(math.rad(llp)) * 4 - 0.5, math.sin(math.rad(llp)) * 9)
                PlaySound(fireSFX)
            end
        end
    end
    ...
    if cartype == 4 then
        ...
        if Globals.difficulty >= GameDifficulty.Hard and firewait == 20 then
    ...
    if self.position.x <= 540+100 and (not spawnedNextCarriage) and cartype ~= 4 then
        SpawnNextCart(800)
    end
    ...
    if (self.position.x < -150-100 and cartype ~= 4) or self.hitPoints <= 0 then
        if self.position.x >= 540+100 and (not spawnedNextCarriage) and cartype ~= 4 then
...
function SpawnNextCart(xPosition)
    ...
    nextCarriageID = SpawnEntityLocal(entity, {x=xPosition+100, y=-480}, nil);
...
function OnRender()
    ...
    if cartype == 1 then
        bigBarrelAnimator.position = {x=-3, y=103 + 15 - barreloff}
    end
